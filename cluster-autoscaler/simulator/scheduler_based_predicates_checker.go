...
func (p *SchedulerBasedPredicateChecker) CheckPredicates1(clusterSnapshot ClusterSnapshot, pod *apiv1.Pod, nodeName string) *PredicateError {
	if clusterSnapshot == nil {
	...
	klog.V(0).Infof("nodeLabels ",nodeInfo.Node().Labels)	
    if err != nil {
...
func (p *SchedulerBasedPredicateChecker) CheckPredicates(clusterSnapshot ClusterSnapshot, pod *apiv1.Pod, nodeName string) *PredicateError {
	//klog.V(0).Infof("nodeName ",nodeName)
	if clusterSnapshot == nil {
		//klog.V(0).Infof("err1 ")
		return NewPredicateError(InternalPredicateError, "", "ClusterSnapshot not provided", nil, emptyString)
	}
