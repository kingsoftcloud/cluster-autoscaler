...
import (
	"bytes"
	"fmt"
	"k8s.io/autoscaler/cluster-autoscaler/cloudprovider/kce"
	"math"
	"strings"
	"time"
...
var (
	notFitMultiSubnetBalancePolicy = &skippedReasons{[]string{"not fit multi-subnet balance policy for scale-up"}}
)
...
func EstimateZoneLabelForScaleUp(pods []*apiv1.Pod, nodeGroup cloudprovider.NodeGroup) bool {
	var skip bool
	var podWithZoneLabel bool
	var nodeGroupWithZoneLabel bool
	var podZoneLabel string

	nodeGroupId := nodeGroup.Id()////asg.name
	_, nodeGroupZoneLabel := kce.ParseNodeGroupId(nodeGroupId)////////////asg.name.id,,asg.name.zone 获取伸缩组的label信息
	nodeGroupWithZoneLabel = nodeGroupZoneLabel != ""

	for _, pod := range pods {
		if zone, found := pod.Spec.NodeSelector[apiv1.LabelZoneFailureDomain]; found {//LabelFailureDomainBetaZone="failure-domain.beta.kubernetes.io/zone"
			podWithZoneLabel = true
			podZoneLabel = zone
			break
		}
	}
	klog.V(2).Infof("podWithZoneLabel,nodeGroupWithZoneLabel ", podWithZoneLabel, nodeGroupWithZoneLabel,)
	if !podWithZoneLabel && nodeGroupWithZoneLabel {
		skip = true
	}
	...
	if podWithZoneLabel && nodeGroupWithZoneLabel && nodeGroupZoneLabel != podZoneLabel {
		skip = true
	}

	if skip {//skip = true
		klog.Infof("### EstimateZoneLabelForScaleUp: ASG-%s not match Pod with nodeSelector %s=%s , will skip it.", nodeGroupId, apiv1.LabelZoneFailureDomain, podZoneLabel)
	} else { //skip = false
		klog.Infof("### EstimateZoneLabelForScaleUp: ASG-%s checked success. Pod nodeSelector %s=%s", nodeGroupId, apiv1.LabelZoneFailureDomain, podZoneLabel)
	}
...
func ScaleUp(context *context.AutoscalingContext, processors *ca_processors.AutoscalingProcessors, clusterStateRegistry *clusterstate.ClusterStateRegistry, unschedulablePods []*apiv1.Pod,
	...
	for _, nodeGroup := range nodeGroups {
		//change
		//if EstimateZoneLabelForScaleUp(unschedulablePods, nodeGroup) {
		//	klog.V(3).Infof("Skipping ASG %s - not ready for scaleup - current asg not fits KCE scale up policy", nodeGroup.Id())
		//	skippedNodeGroups[nodeGroup.Id()] = notFitMultiSubnetBalancePolicy
		//	continue
		//}
